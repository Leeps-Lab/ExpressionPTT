		...
		for row in reader:
			if (rowNum != 0):
				...
				try:
					...
					# if the message is the accumulated points message, let's continue.
					# this message decides to encode the json parameter as a single int 0, which
					# breaks the rest of the parser. 
					if (message == '_accumulated_points'): continue;
					...
					if ("tmp" in json_object):
						...
						for userInfo in json_object["tmp"]:
							...
							data = [[message, period, messageTime, current_slot, user, group, ctr, revenue, bid, vfc, cost, payoff]]
							writer.writerows(data)
