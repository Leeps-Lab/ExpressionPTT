Redwood.controller("SubjectCtrl", ["$rootScope", "$scope", "RedwoodSubject", 'SynchronizedStopWatch', function($rootScope, $scope, rs, SynchronizedStopWatch) {
	...
	function VCG() {
	  ...
	  for(var k = 0; k < network.bidders.length; k++) {
	    for(var i = k; i < network.bidders.length - 1; i++) {
	      ...
	      /*if(bids[j + 1] == null) vcg[k] = 0;
	      else if(ctr[j + 1] == null) vcg[k] += ctr[j] * bids[j + 1][1];
	      else vcg[k] += ctr[j] * bids[j + 1][1] - ctr[j + 1] * bids[j + 1][1];*/

	      	if(network.bidders[j + 1].bid === null) {
		      	network.bidders[k].cost = 0;
	      	} else if(network.bidders[j + 1].ctr === null) {
		        network.bidders[k].cost += network.bidders[j].ctr * network.bidders[j + 1].bid;
	      	} else {
		       	network.bidders[k].cost += network.bidders[j].ctr * network.bidders[j + 1].bid -
		        network.bidders[j + 1].ctr * network.bidders[j + 1].bid;
			}
	    }
	  }
	}
	...
	*/
	function replace_bid(id, val){
		console.log("REPLACING BID FOR id: " + id);
	...
	/* checks if all palyers have made a move yet */
	function game_started(){
	  var out = 0;
	  for(var i = 0; i < network.bidders.length; ++i){
	    if(network.bidders[i].bid === 0) ++out;
	  }
	...
	rs.on_load(function() {
		...
		//prevent user input while period syncs up
	 	...
	 	if (!chosen) {
	  	...
	  	chosen = true;
	  	//console.log("MY ID:" + id + " Time_keeper:" + time_keeper);
    	if(id === time_keeper){

	    	//$("#nav").append('<li><span>TIME KEEPER</span></li>');
	    }
	    //setInterval(tick, 500);
	    //setInterval(track, 100);

	    //setInterval(update_graphs_at_interval, 1000);
	    //setInterval(log, 500);
	    
	    //setInterval(sync_net, 100);
		...
		$("#sub_bid").click(function(){
		    ...
		    console.log("Clicking SUBMIT BID");
		    if(input != last_bid){
		      ...
		      if(input !== null && last_bid != input && input <= max_bid) { 
		        ...
		        if(id == time_keeper){
		          	replace_bid(id, Number(new_bid)); //this doesn't get sent normally (?)
		...
		$("[data-slider]").each(function () {
		    ...
		    //document.getElementById('bidamnt').value = document.getElementById('hbid').value;
		    update_flow_graph();
...
*/
	...
	function update_ct_graph() {
  	...
  	function log(){
    	if(waiting) return;
    	if(redwood_debug) last_bid = Math.random()*100;
    	...
    	if(id == time_keeper){
      		...
      		var value = network.bidders;
      		//rs.send("data_log", { value:value });
      		//var tmp = network.bidders;
      		//rs.send("sync_net", { value:value });
    	}
  	}
  	/*
  	function sync_net() {
	...
	function tick() {
	    ...
	    //if(time_keeper === null || time_keeper === undefined) time_keeper = "Subject-1@leeps.ucsc.edu";
  	...
  	function update_graphs_at_interval() {
  	...
  	rs.recv("update_bids", function (uid, msg) {
    	if(msg.bidss !== null) {
      		for(var i = 0; i < network.bidders.length; ++i){
        		network.bidders[i].bid = Number(msg.bidss[i]);
      		}
    	}
  	});
  	...
  	rs.recv("new_period", function (uid, msg) {
    	...
    	//clearInterval(log);
    	//clearInterval(tick);
    	clearInterval(update_graphs_at_interval);
    	$("#myModal").modal('show');    
    	//if(msg.Value.current_period !== null) r.set_period(msg.Value.current_period + 1);
  	});
  
 	rs.recv("change_auction_type", function(uid, msg) {
 		console.log("RECEIVING CHANGE AUC TYPE");
 		console.log("MSG == ");
 		console.log(msg);
    	if (msg.auction_type == "GSP") {
    		auction_func = GSP;
    		console.log("Auction Function is now GSP");
    	} else if (msg.auction_type == "VCG") {
    		auction_func = VCG;
    		console.log("Auction Function is now VCG");
    	}
  	});

  	/* enter new subject bid */
  	rs.recv("replace_bids", function(uid, msg) {
  		console.log("receiving replace bids!");
    	if(msg.new_bid !== null && msg.new_bid <= max_bid){
	      	//var ind = msg.Value.new_bid.toString().indexOf('.');
	      	/*if(ind == -1)
	        	replace_bid(msg.Value.id, Number(msg.Value.new_bid));
	      	else
	        	replace_bid(msg.Value.id, Number(msg.Value.new_bid).toPrecision(ind + 2));*/
  	...
  	rs.recv("config", function(uid, msg) {
    	...
    	if(!chosen) {
    		//time_keeper = in_group[Math.floor(Math.random()*in_group.length)];
    		console.log("Choosing time keeper in CONFIG");
    	}


    	console.log("Time keeper has been chosen! " + time_keeper);
    	chosen = true;
